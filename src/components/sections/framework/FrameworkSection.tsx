...
export function FrameworkSection() {
  ...
  return (
    <section 
      ref={containerRef}
      className="relative min-h-screen bg-[#020617] py-24 px-6 overflow-hidden flex items-center justify-center"
    >
      {/* Background Depth */}
        ...
        <div className="relative w-full aspect-square max-w-[600px] flex items-center justify-center">
          ...
          <div className="relative z-30 group cursor-default">
            ...
            <motion.div 
              ...
              className="absolute -inset-32 border border-gold/10 rounded-full"
            />
            ...
            <motion.div 
              ...
              className="relative w-24 h-24 md:w-32 md:h-32 bg-navy border border-gold/30 rounded-full flex items-center justify-center z-40 backdrop-blur-xl"
            >
              <img src="/mooviagold.svg" alt="MOOVIA" className="w-12 md:w-16 h-auto" />
          ...
          <svg className="absolute inset-0 w-full h-full z-20 overflow-visible" viewBox="0 0 100 100">
            ...
            {dimensions.map((_, i) => (
               <motion.circle
                  ...
                  style={{ offsetPath: "path('M 50 15 Q 85 15 85 50 Q 85 85 50 85 Q 15 85 15 50 Q 15 15 50 15')" }}
               />
            ))}
          </svg>
            ...
            return (
              <div 
                ...
                style={{ 
                  ...
                  opacity: !isAnyHovered || isHovered ? 1 : 0.3
                }}
              ...
              >
                ...
                <motion.div 
                  ...
                  className={`
                    relative w-14 h-14 md:w-20 md:h-20 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-500
                    ${isHovered ? 'bg-gold border-gold text-black shadow-[0_0_30px_rgba(173,137,87,0.4)]' : 'bg-navy/80 border-gold/30 text-gold shadow-[0_0_20px_rgba(0,0,0,0.5)]'}
                    backdrop-blur-md
                  `}
                >
                  <Icon className={`w-6 h-6 md:w-8 md:h-8 transition-colors duration-500 ${isHovered ? 'text-black' : 'text-gold'}`} strokeWidth={1.2} />
                  ...
                  {/* ID Tag */}
                  <div className="absolute -top-1 -right-1 w-5 h-5 md:w-6 md:h-6 bg-gold text-black rounded-full flex items-center justify-center font-display text-[9px] md:text-[10px] font-bold border-2 border-navy">
                    {dim.id}
                  </div>
                </motion.div>
                ...
                <div 
                  className={`
                    ...
                    w-[220px] md:w-[320px]
                    ${dim.textPosition === 'top' ? 'bottom-[120%] left-1/2 -translate-x-1/2 text-center' : ''}
                    ${dim.textPosition === 'bottom' ? 'top-[120%] left-1/2 -translate-x-1/2 text-center' : ''}
                    ${dim.textPosition === 'right' ? 'left-[120%] top-1/2 -translate-y-1/2 text-left' : ''}
                    ${dim.textPosition === 'left' ? 'right-[120%] top-1/2 -translate-y-1/2 text-right' : ''}
                  `}
                >
                  <div className={`transition-all duration-500 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-80 md:opacity-40'}`}>
                    <p className="font-body text-[10px] tracking-[0.2em] uppercase text-gold mb-1">{dim.verb}</p>
                    <h3 className="font-display text-[18px] md:text-[24px] text-white font-[200] mb-2">{dim.title}</h3>
                    ...
                    <AnimatePresence>
                      {(isHovered || !isAnyHovered) && (
                        ...
                        >
                          <p className="font-body text-[12px] md:text-[14px] text-w35 leading-relaxed font-[300] mb-4">
                            {dim.description}
                          </p>
                          <p className="font-display text-[11px] md:text-[13px] text-gold italic font-[300]">
                            {dim.quote}
                          </p>
                        </motion.div>
                      )}
