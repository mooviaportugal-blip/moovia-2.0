...
import { Plus, Building2, Mail, Trash2, ShieldCheck } from "lucide-react";
...
function CompaniesAdmin() {
    ...
    return (<div className="p-8 space-y-8">
          ...
          <DialogContent className="bg-black-2 border-b18 text-white max-w-md">
            ...
            <div className="space-y-6 pt-4">
              <div className="space-y-3">
                ...
                <RadioGroup defaultValue="company" onValueChange={(v) => setUserType(v)} className="flex gap-4">
              ...
              {userType === "company" ? (<div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="companyName" className="text-[10px] uppercase tracking-widest text-w35">Nome da Empresa</Label>
                    <Input id="companyName" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="bg-black-3 border-b18 text-white focus:border-gold"/>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(v) => setFormData({ ...formData, expatriateId: v })}>
                      ...
                      <SelectContent className="bg-black-2 border-b18 text-white">
                        {expatriates.map(exp => (<SelectItem key={exp.id || exp.email} value={exp.id || exp.email} className="focus:bg-gold focus:text-black">
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="exemplo@empresa.com" className="bg-black-3 border-b18 text-white focus:border-gold"/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="pass" className="text-[10px] uppercase tracking-widest text-w35">Password Temporária</Label>
                <Input id="pass" type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="••••••••" className="bg-black-3 border-b18 text-white focus:border-gold"/>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (<p className="text-w35">Carregando...</p>) : companies.length === 0 ? (<p className="text-w35">Nenhuma empresa encontrada.</p>) : (companies.map(company => (<Card key={company.id} className="bg-black-2 border-b18 group">
