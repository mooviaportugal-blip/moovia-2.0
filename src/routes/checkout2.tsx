      ...
      <div className="mx-auto max-w-[1200px] px-6 lg:px-12 py-10 md:py-16">
        ...
        {search.status && (
          <StatusBanner
            ...
            onReset={() => router.navigate({ to: "/checkout2", search: { status: undefined, name: search.name, email: search.email } })}
          />
        )}
                  ...
                  onSubmit={async ({ formData, selectedPaymentMethod }) => {
                    ...
                    try {
                      ...
                      if (result.status === "approved") {
                        router.navigate({ to: "/checkout2", search: { status: "success", name: search.name, email: search.email } });
                        return;
                      }
