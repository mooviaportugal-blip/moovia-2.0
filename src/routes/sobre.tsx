    ...
    <section className="bg-black-2 py-[120px] px-6 lg:px-20 border-t border-b18">
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="mt-24 pt-16 border-t border-b18">
          ...
          <h3 className="font-display font-[200] text-white leading-[1.1] tracking-[-0.03em] text-[clamp(24px,3vw,38px)] mb-8 max-w-[900px]">
            Porque é que a MOOVIA precisa de uma Base de Conhecimento e não apenas de um{" "}
            <span className="italic text-gold-l">questionário.</span>
          </h3>

          <div className="max-w-[820px] space-y-5 font-body font-[300] text-[15px] md:text-[16px] text-w35 leading-[1.9]">
            <p>
              O objetivo da MOOVIA não é apenas recolher informação sobre um expatriado. É construir um sistema capaz de transformar centenas de pequenas informações em recomendações objetivas, consistentes e escaláveis.
            </p>
            <p>
              O motor de decisão depende da qualidade e da profundidade dos dados recolhidos. Quanto melhores forem os dados de entrada, melhores serão as recomendações produzidas. Por isso não estamos a construir um questionário, mas uma Base de Conhecimento Estruturada da Mobilidade Internacional.
            </p>
          </div>
          ...
          <div className="mt-14 grid lg:grid-cols-2 gap-6 lg:gap-8">
            <div className="border border-b18 bg-w05 p-8">
              <p className="font-body text-[11px] tracking-[0.28em] uppercase text-gold mb-5">O princípio</p>
              <p className="font-body text-[14px] font-[300] text-w35 leading-[1.85] mb-5">
                Em vez de perguntar apenas “Tem filhos?”, o sistema precisa de compreender dezenas de fatores relacionados com essa realidade.
              </p>
              <ul className="space-y-2">
                {[
                  "Quantos filhos e que idades têm",
                  "Necessitam de escola internacional",
                  "Falam inglês ou precisam de apoio pedagógico",
                  "Praticam desporto e que tempo de deslocação aceitam",
                  "Precisam de transporte escolar",
                  "Existem necessidades especiais",
                  "É importante manter continuidade curricular",
                ].map((item) => (
                  <li key={item} className="flex gap-3 font-body text-[13.5px] font-[300] text-w35 leading-[1.7]">
                    <span className="text-gold/70 mt-[2px]">·</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
              <p className="mt-6 font-body text-[13.5px] font-[300] text-white/70 leading-[1.8]">
                Cada resposta representa uma variável. São estas variáveis que alimentam o motor de decisão.
              </p>
            </div>

            {/* Exemplo de variável */}
            <div className="border border-gold/25 bg-w05 p-8">
              <p className="font-body text-[11px] tracking-[0.28em] uppercase text-gold mb-5">Pensamos em variáveis</p>
              <span className="inline-block font-body text-[12px] tracking-[0.18em] text-gold-l border border-gold/30 px-3 py-1 mb-5">
                HOU_034
              </span>
              <dl className="space-y-4">
                {[
                  { k: "Pergunta", v: "Qual a importância de receber luz natural durante a manhã?" },
                  { k: "Resposta", v: "Escala de 0 a 10" },
                  { k: "Peso", v: "3" },
                  { k: "Regra", v: "Se maior que 8, priorizar imóveis orientados a nascente. Se menor que 4, ignorar este critério." },
                ].map((r) => (
                  <div key={r.k} className="border-l border-gold/30 pl-4">
                    <dt className="font-body text-[11px] tracking-[0.22em] uppercase text-w35 mb-1">{r.k}</dt>
                    <dd className="font-body text-[14px] font-[300] text-white/85 leading-[1.7]">{r.v}</dd>
                  </div>
                ))}
              </dl>
              <p className="mt-6 font-body text-[13.5px] font-[300] text-w35 leading-[1.8]">
                Cada pergunta deixa de ser apenas uma pergunta. Passa a ser um dado estruturado que pode ser utilizado centenas de vezes.
              </p>
            </div>
          </div>
          ...
          <div className="mt-16">
            <p className="font-body text-[11px] tracking-[0.28em] uppercase text-gold mb-8">
              Como chegamos às 700 a 900 variáveis
            </p>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-b18 border border-b18">
              {[
                {
                  n: "Fase 1",
                  t: "Brainstorming",
                  d: "Reunir, sem filtros, tudo o que pode influenciar o sucesso de uma mobilidade internacional: filhos, animais, desporto, religião, escolas, clima, transportes, alimentação, habitação, acessibilidade, orçamento.",
                },
                {
                  n: "Fase 2",
                  t: "Organização por dimensões",
                  d: "Agrupamento em Família, Habitação, Educação, Saúde física, Documentação, Mobilidade, Financeiro, Lifestyle, Comunidade, Serviços, Segurança e Infraestrutura. Cada dimensão passa a ter dezenas de variáveis.",
                },
                {
                  n: "Fase 3",
                  t: "Perguntas estruturadas",
                  d: "Cada variável é convertida numa pergunta em escala de 0 a 10, resposta sim ou não, ou lista fechada. Evitamos respostas abertas porque o sistema trabalha melhor com dados normalizados.",
                },
                {
                  n: "Fase 4",
                  t: "Definição das regras",
                  d: "Definimos peso, prioridade, dependências, impacto e recomendações. Se existem filhos com menos de 10 anos e ensino internacional é prioritário, a procura de habitação privilegia zonas próximas das escolas internacionais.",
                },
                {
                  n: "Fase 5",
                  t: "Validação especializada",
                  d: "Cada especialista valida a sua área: aspetos humanos, documentação e enquadramento legal, mobilidade, risco e metodologia, e implementação tecnológica.",
                },
                {
                  n: "Resultado",
                  t: "Sistema de conhecimento",
                  d: "Cada resposta alimenta dezenas de regras, cada regra contribui para um índice de risco e cada índice produz recomendações objetivas, ajustadas continuamente à medida que acompanhamos mais famílias.",
                },
              ].map((f, i) => (
                <motion.div
                  key={f.n}
                ...
                >
                  <span className="block font-body text-[11px] tracking-[0.24em] uppercase text-gold mb-3">{f.n}</span>
                  <h4 className="font-display text-[1.05rem] font-[300] text-white mb-3">{f.t}</h4>
                  <p className="font-body text-[13.5px] font-[300] text-w35 leading-[1.75]">{f.d}</p>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-16 border-l-2 border-gold/50 pl-6 md:pl-8 max-w-[860px]">
            <p className="font-body text-[11px] tracking-[0.28em] uppercase text-gold mb-4">Vantagem competitiva</p>
            <p className="font-body font-[300] text-[15px] md:text-[16px] text-w35 leading-[1.9] mb-4">
              Qualquer empresa consegue criar um formulário. Muito poucas conseguem criar uma base de conhecimento estruturada sobre mobilidade internacional.
            </p>
            <p className="font-body font-[300] text-[15px] md:text-[16px] text-white/80 leading-[1.9]">
